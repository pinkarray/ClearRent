...
class _LandlordHomeScreenState extends State<LandlordHomeScreen> {
          ...
          (doc) {
            ...
            switch (statusStr) {
              ...
              case 'rejected':
                ...
                break;
              case 'expired':
                vStatus = VerificationStatus.expired;
                break;
              default:
                    ...
                    children: [
                      ...
                      if (_profileImageUrl != null)
                        _buildSourceOption(
                          ...
                          label: 'Remove',
                          onTap:
                              () => Navigator.pop(ctx, null), // null = remove
                ...
                snapshot.docs.map((doc) {
                  ...
                  for (final key in ['createdAt', 'updatedAt']) {
                    if (data[key] is Timestamp) {
                      data[key] =
                          (data[key] as Timestamp).toDate().toIso8601String();
                    }
                  }
  ...
  void _startLinkedTenantsStream() {
    final uid = FirebaseAuth.instance.currentUser?.uid;
    if (uid == null) {
      setState(() => _isLoadingLinkedTenants = false);
      return;
    }
    ...
    if (activity.propertyId != null) {
      ...
      if (property != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text(
              'Could not open that property. Please try again.',
            ),
  ...
  IconData _getActivityIcon(ActivityType type) {
    switch (type) {
      ...
      case ActivityType.issueDisputed:
        return Icons.warning_amber_outlined;
      case ActivityType.issueConfirmed:
        return Icons.check_circle_outline;
      case ActivityType.inspectionRequest:
        return Icons.search_outlined;
      case ActivityType.inspectionApproved:
        return Icons.event_available_outlined;
      case ActivityType.inspectionDeclined:
        return Icons.event_busy_outlined;
      case ActivityType.inspectionCompleted:
        return Icons.done_all_outlined;
      case ActivityType.inspectionRated:
        return Icons.star_outline;
      case ActivityType.payoutReceived:
        return Icons.account_balance_wallet_outlined;
      case ActivityType.moveoutRequested:
        return Icons.logout;
      case ActivityType.moveoutCompleted:
        return Icons.logout;
    }
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color:
                isExpiring ? AppColors.warning.withAlpha(77) : AppColors.border,
          ),
        ),
        child: Row(
          children: [
            ...
            ClipRRect(
              borderRadius: BorderRadius.circular(10),
              child:
                  rental.propertyImage.isNotEmpty
                      ? CachedNetworkImage(
  ...
  Widget _buildLinkedTenantCard(TenancyLinkModel link) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: isDueSoon ? AppColors.warning.withAlpha(77) : AppColors.border,
        ),
      ),
      child: Row(
        children: [
  ...
  Widget _buildPropertiesTab() {
    return Column(
      ...
      children: [
        Padding(
          padding: const EdgeInsets.all(20),
          child: Text('My Properties', style: AppTextStyles.h2),
        ),
        Expanded(
          child:
              _isLoadingProperties
  ...
  Future<void> _togglePropertyStatus(PropertyModel property) async {
    await _propertyService.updateAvailability(
      property.id,
                  ...
                  children: [
                    ...
                    Expanded(
                      child: _ProfileStat(
                        ...
                        value:
                            _landlordRating > 0
                                ? _landlordRating.toStringAsFixed(1)
                                : 'N/A',
                        label:
                            'Rating${_totalRatings > 0 ? ' ($_totalRatings)' : ''}',
                        color: AppColors.warning,
                      ),
                    ),
                  ],
                ...
                _ProfileSection(
                  ...
                  items: [
                    ...
                    _ProfileMenuItem(
                      ...
                      subtitle:
                          _totalEarnings > 0
                              ? '₦${(_totalEarnings / 1000000).toStringAsFixed(1)}M total'
                              : 'View your earnings and payment history',
                      onTap: () => context.push('/landlord/earnings'),
                    ),
                    _ProfileMenuItem(
                      icon: Icons.receipt_long_outlined,
                      title: 'Payments & Documents',
                      subtitle: 'Receipts and payment history',
                      onTap: () => context.push('/landlord/documents'),
                    ),
                  ],
                ),
                const SizedBox(height: 24),
  ...
  String _getVerificationSubtitle() {
    switch (_verificationStatus) {
      case VerificationStatus.none:
        return 'Tap to get verified';
      case VerificationStatus.pending:
        return 'Under review';
      case VerificationStatus.verified:
        return 'Identity verified';
      case VerificationStatus.rejected:
        return 'Verification failed - tap to retry';
      case VerificationStatus.expired:
        return 'Verification expired - tap to renew';
    }
  }
...
class _ViewerSheetState extends State<_ViewerSheet> {
  ...
  bool get _isVerified => _viewerProfile?['verificationStatus'] == 'verified';

  Future<void> _startConversation() async {
    if (widget.activity.propertyId == null) return;
    setState(() => _isMessaging = true);
    try {
      final property = await widget.propertyService.getProperty(
        widget.activity.propertyId!,
      );
            ...
            if (_isLoadingProfile)
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(
                    _isMessaging ? 'Opening chat...' : 'Send a Message',
                  ),
...
class _ActivityItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color:
            isUnread ? AppColors.primaryLight.withAlpha(26) : AppColors.surface,
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: isUnread ? AppColors.primary.withAlpha(77) : AppColors.border,
        ),
      ),
      child: Row(
        children: [
