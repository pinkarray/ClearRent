...
class _VerificationCenterScreenState extends State<VerificationCenterScreen> {
  ...
  String _accountType = 'landlord'; // landlord, tenant, or agent
  bool _isLoading = true;
  bool _isSubmitting = false;

  // Document files - organized by type
  // Common
  File? _ninFile;
        ...
        setState(() {
          switch (type) {
            ...
            case 'utilityBill':
              ...
              break;
            case 'proofOfIncome':
              _proofOfIncomeFile = File(image.path);
              break;
            case 'proofOfAddress':
              _proofOfAddressFile = File(image.path);
              break;
            case 'guarantorId':
              _guarantorIdFile = File(image.path);
              break;
            case 'experienceProof':
              _experienceProofFile = File(image.path);
              break;
          }
        });
      }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 24),
          Text(
            'You\'re a verified ${_getUserTypeLabel()}!',
            style: AppTextStyles.h2,
          ),
          const SizedBox(height: 12),
          Text(
            _getVerifiedDescription(),
  ...
  String _getVerifiedDescription() {
    switch (_accountType) {
      case 'landlord':
        return 'Your identity and property ownership have been confirmed. Tenants can trust that you\'re a legitimate landlord.';
      case 'tenant':
        return 'Your identity and income have been verified. Landlords can trust that you\'re a reliable tenant.';
      case 'agent':
        return 'Your identity has been verified. You can now receive property inspection assignments from landlords.';
      default:
      ...
      case 'tenant':
        return [
          _buildBenefitItem(
            Icons.thumb_up,
            'Preferred Tenant',
            'Landlords prioritize verified tenants',
          ),
          const SizedBox(height: 16),
          _buildBenefitItem(
            Icons.speed,
            'Faster Approval',
            'Get approved for properties more quickly',
          ),
          const SizedBox(height: 16),
          _buildBenefitItem(
            Icons.verified_user,
            'Trust Badge',
            'Your verified status shows on your profile',
          ),
        ];
      case 'agent':
        return [
          _buildBenefitItem(
            Icons.assignment,
            'Receive Assignments',
            'Landlords can now assign properties to you',
          ),
          const SizedBox(height: 16),
          _buildBenefitItem(
            Icons.monetization_on,
            'Earn Money',
            'Get paid for conducting property inspections',
          ),
          const SizedBox(height: 16),
          _buildBenefitItem(
            Icons.star,
            'Build Reputation',
            'Collect ratings to attract more assignments',
          ),
        ];
      default:
        return [];
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            '${_getUserTypeLabel()} Documents',
            style: AppTextStyles.h4,
          ),
          const SizedBox(height: 8),
          Text(
            _getUploadDescription(),
            style: AppTextStyles.bodySmall.copyWith(
              color: AppColors.textSecondary,
            ),
          ),
          ...
          // Document upload cards based on account type
  ...
  String _getWhyVerifyText() {
    switch (_accountType) {
      case 'landlord':
        return 'Verified landlords get more inquiries and tenant trust.';
      case 'tenant':
        return 'Verified tenants get faster approval from landlords.';
      case 'agent':
        return 'Verified agents can receive property inspection assignments.';
      default:
        return 'Get verified to unlock all features.';
    }
  }
  ...
  String _getUploadDescription() {
    switch (_accountType) {
      case 'landlord':
        return 'We need these documents to verify your identity and property ownership.';
      case 'tenant':
        return 'We need these documents to verify your identity and ability to pay rent.';
      case 'agent':
        return 'We need these documents to verify your identity and establish trust with landlords.';
      default:
  ...
  List<Widget> _buildDocumentUploadCards() {
    switch (_accountType) {
      case 'landlord':
        return _buildLandlordDocs();
      case 'tenant':
        return _buildTenantDocs();
      case 'agent':
        return _buildAgentDocs();
      default:
        return [];
    }
  }
  ...
  List<Widget> _buildTenantDocs() {
    return [
      ...
      _DocumentUploadCard(
        ...
        file: _proofOfIncomeFile,
        onTap: () => _pickDocument('proofOfIncome'),
        onRemove: () => setState(() => _proofOfIncomeFile = null),
      ),
    ];
  }
  ...
  List<Widget> _buildAgentDocs() {
    return [
      ...
      _DocumentUploadCard(
        ...
        file: _proofOfAddressFile,
        onTap: () => _pickDocument('proofOfAddress'),
        onRemove: () => setState(() => _proofOfAddressFile = null),
      ),
      const SizedBox(height: 24),
      ...
      _DocumentUploadCard(
        ...
        whatWeNeed: 'Any document showing your experience in property/real estate. This helps build trust.',
        icon: Icons.workspace_premium_outlined,
        file: _experienceProofFile,
        onTap: () => _pickDocument('experienceProof'),
        onRemove: () => setState(() => _experienceProofFile = null),
        isOptional: true,
      ),
    ];
  }
      ...
      child: Column(
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Someone who can vouch for your trustworthiness',
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    'Your guarantor should be someone with a stable job or business who knows you personally. We may contact them to verify.',
          ...
          _buildTextField(
            ...
            hint: 'Enter their home or work address',
          ...
          _DocumentUploadCard(
            ...
            subtitle: 'Photo of your guarantor\'s NIN or Government ID',
            whatWeNeed: 'Clear photo showing their name and photo.',
            icon: Icons.badge_outlined,
            file: _guarantorIdFile,
            onTap: () => _pickDocument('guarantorId'),
            onRemove: () => setState(() => _guarantorIdFile = null),
            compact: true,
          ),
        ],
      ),
